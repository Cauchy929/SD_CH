...
#subprocess.check_call(['pip', 'uninstall', 'torch'])
#subprocess.check_call(['pip', 'install', 'torch==1.13.1'])
import nltk
nltk.download('punkt')
...
def create_toprow(is_img2img):
        ...
        with gr.Column(elem_id=f"{id_part}_prompt_container", scale=6):
            with gr.Row():
                with gr.Column(scale=80):
                    with gr.Row():
                        prompt = gr.Textbox(label="Prompt", elem_id=f"{id_part}_prompt", show_label=False, lines=3, placeholder="Prompt (press Ctrl+Enter or Alt+Enter to generate)", width=1)
            
                    with gr.Row():
                        improve = gr.Button('Improve⬇️', elem_id=f"{id_part}_improve", width=0.5)
                        accept = gr.Button('accept⬆️', elem_id=f"{id_part}_accept", width=0.5)

                    with gr.Row():
                        processed_prompt = gr.Textbox(label="Processed Prompt", elem_id=f"{id_part}_pro_prompt", show_label=False, lines=3, placeholder="Processed Prompt (press Improve button to generate)", width=1, readonly=False)

                    with gr.Row():
                        negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"{id_part}_neg_prompt", show_label=False, lines=2, placeholder="Negative prompt (press Ctrl+Enter or Alt+Enter to generate)", width=1)
    ...
    return prompt, prompt_styles, improve, accept, processed_prompt, negative_prompt, submit, button_interrogate, button_deepbooru, prompt_style_apply, save_style, paste, extra_networks_button, token_counter, token_button, negative_token_counter, negative_token_button
...
def old_improve_prompt(prompt):
    # 在这里编写处理逻辑，对 prompt 进行处理，返回处理后的字符串
    # 分词
    ...
    processed_text = [word for word, tag in tagged_words if tag.startswith('NN')]
...
def improve_prompt(prompt):
    ...
    for phrase in noun_phrases:
        ...
        for word, tag in tagged_words:
            if tag.startswith('NN'):  # 检查词性标记是否以"NN"开头，表示名词
    ...
    line1 = "((Chinese ancient painting))\n"
    line3 = "\nnatural light, outdoor, gentle color, (professional), high resolution"

    #prompt = [line1]
    #prompt.extend(line2)
    #prompt.append(line3)
    
    prompt = ", ".join([line1] + [line2] + [line3])
    #prompt = ", ".join(prompt)

    return prompt

def create_long_prompt(old_prompt):
    line1 = "Chinese painting"
    line3 = "natural light, outdoor, gentle color, professional, high resolution"

    prompt = ", ".join([line1] + old_prompt + [line3])
    return(prompt)

def paste_prompt(prompt):
    return prompt
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        #创建UI组件
        txt2img_prompt, txt2img_prompt_styles, txt2img_improve, txt2img_accept, txt2img_pro_prompt, txt2img_negative_prompt, submit, _, _, txt2img_prompt_style_apply, txt2img_save_style, txt2img_paste, extra_networks_button, token_counter, token_button, negative_token_counter, negative_token_button = create_toprow(is_img2img=False)
        txt2img_improve.click(
            fn=improve_prompt,
            inputs=[txt2img_prompt],
            outputs=[txt2img_pro_prompt]
        )  # 将事件处理函数绑定到按钮的点击事件
        txt2img_accept.click(
            fn=paste_prompt,
            inputs=[txt2img_pro_prompt],
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        img2img_prompt, img2img_prompt_styles,img2img_improve, img2img_accept, img2img_pro_prompt, img2img_negative_prompt, submit, img2img_interrogate, img2img_deepbooru, img2img_prompt_style_apply, img2img_save_style, img2img_paste, extra_networks_button, token_counter, token_button, negative_token_counter, negative_token_button = create_toprow(is_img2img=True)
